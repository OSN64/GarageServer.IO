...
GarageServerGame.prototype.getPlayer = function (socket) {
    for (var idx = 0; idx < this.playerController.entities.length; idx ++) {
        if (this.playerController.entities[idx].id === socket.id) {
            return this.playerController.entities[idx];
        }
    }
};
